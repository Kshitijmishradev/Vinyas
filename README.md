...
- Enforces layer boundaries and configurable budgets through `vinyas.yaml`.
...
vinyas analyze . --output analysis.json
vinyas report . --format html --output vinyas-report.html
vinyas check . --baseline main --sarif vinyas.sarif
```
...
Open `http://127.0.0.1:5173`. The API accepts only the repository passed to `vinyas serve`; the browser cannot request arbitrary host paths.
...
```bash
VINYAS_REPO=/absolute/path/to/another/repository docker compose up --build
```

For a persistent local setting, copy `.env.example` to `.env`, set `VINYAS_REPO`, and then use the normal `docker compose up --build` command. After changing repositories, restart the stack with `docker compose down` followed by `docker compose up --build`.

## CLI

```text
vinyas analyze PATH [--config FILE] [--output FILE]
vinyas report PATH --format html|json|sarif --output FILE
vinyas check PATH --baseline GIT_REF_OR_JSON [--sarif FILE] [--html FILE]
vinyas serve PATH [--host 127.0.0.1] [--port 8000]
```

`vinyas check` exits with status 1 only when the current repository contains active finding fingerprints absent from the baseline. The Git ref must be available locally; CI should use a full checkout or fetch the base ref. The legacy `architect` executable remains available as a compatibility alias for this release.

## Governance configuration

Copy `vinyas.example.yaml` to `vinyas.yaml` and adapt it. Existing `architect.yaml` files are still recognized when `vinyas.yaml` is absent.
...
```bash
export VINYAS_OLLAMA_ENABLED=true
export VINYAS_OLLAMA_MODEL=qwen2.5-coder:7b
vinyas serve .
```
...
The included `.github/workflows/vinyas.yml` runs backend checks, frontend checks, Docker builds, baseline-aware governance, SARIF upload, and an HTML report artifact. Ensure the workflow fetches the pull request base ref.

```bash
vinyas check . \
  --baseline "$BASE_SHA" \
  --sarif vinyas.sarif \
  --html vinyas-report.html
```
